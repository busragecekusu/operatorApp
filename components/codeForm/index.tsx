...
const CodeForm: React.FC<CodeFormProps> = ({ onNext, email }) => {
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      // After successful verification, use the role and companyId from API
      const tempUser = {
        token: response.data.token || "temp-token",
        user: {
          id: response.data.user?.id || response.data.userId || "temp-id",
          email: userEmail,
          username: response.data.user?.username || response.data.username || userEmail.split('@')[0],
          role: response.data.user?.role || "seller", // API'den gelen rolü kullan
          isActive: true,
          // API'den gelen companyId değerini ekle
          companyId: response.data.user?.companyId || null,
          // API'den gelen diğer kullanıcı bilgilerini de ekleyelim
          sellerId: response.data.user?.sellerId || null,
          operatorId: response.data.user?.operatorId || null,
          phoneNumber: response.data.user?.phoneNumber || null,
        },
      };
      
      console.log("API'den gelen kullanıcı bilgileri:", response.data.user);
      console.log("API'den gelen companyId:", response.data.user?.companyId);
