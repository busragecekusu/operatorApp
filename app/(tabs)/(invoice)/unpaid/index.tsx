...
import styles from "../../../../styles/invoice/unpaidStyles";
...
import AdjustPrice from "@/components/modalContents/contents/adjustPrice";
  ...
  useEffect(() => {
    const fetchInvoices = async () => {
      try {
        ...
        // Filter only UNPAID status invoices (status isn't PAID or PENDING)
  ...
  const actionOptions: Option[] = [
    ...
    },
    {
      ...
      onPress: async () => {
        ...
        try {
          setLoading(true);
          // Seçili tüm faturaları PENDING olarak işaretle
          const updatePromises = selectedInvoices.map(id => 
    ...
    <SafeAreaView style={globalStyles.pageContainer}>
      ...
      >
        <View style={styles.cardWrapper}>
          {invoices.length === 0 ? (
            <Text>No unpaid invoices found</Text>
          ) : (
      ...
      >
        <ReservationModalContent reservationId={currentInvoice?.reservationId || ""} />
      </ModalForm>

      {/* Adjust Price Modal */}
      <ModalForm
        title="Adjust Price"
        visible={activeModal === "adjustPrice"}
      >
        <AdjustPrice />
      </ModalForm>
    </SafeAreaView>
