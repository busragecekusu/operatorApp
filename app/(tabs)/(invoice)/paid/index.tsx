...
import styles from "../../../../styles/invoice/paidStyles";
  ...
  useEffect(() => {
    const fetchInvoices = async () => {
      try {
        ...
        // Filter only PAID status invoices
        const paidInvoices = response.data.filter(invoice => invoice.status === "PAID");
    ...
    <SafeAreaView style={globalStyles.pageContainer}>
      ...
      >
        <View style={styles.cardWrapper}>
          {invoices.length === 0 ? (
            <Text>No paid invoices found</Text>
          ) : (
